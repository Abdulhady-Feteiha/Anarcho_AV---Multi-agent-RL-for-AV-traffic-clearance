...
def getArrivTime(emer,agent):
    dist = CalcDist(emer,agent)
    a = 0.5*(agent.accel-emer.accel)
    b = (agent.spd-emer.spd)
    c = dist
    if a ==0:
        arrivTime = dist/abs(agent.spd-emer.spd)
    else:
        arrivTime = max((-b + sqrt(abs(b**2 - 4*a*c))) / (2 * a),(-b - sqrt(abs(b**2 - 4*a*c))) / (2 * a))
...
if __name__ == "__main__":
    ...
    traci.start([sumoBinary, "-c", "Anarcho1.0.sumocfg",
