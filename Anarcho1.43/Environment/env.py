...
class env():
    ...
    def are_we_done(self, step_number):
        ...
        if(step_number == self.max_steps-1):
            for agent_left in self.list_of_vehicles:
                if (agent_left.control_algorithm_name != "SUMO_KRAUSS"):
                    self.end_q_tables_list.append(agent_left.control_algorithm.q_table)

            if(self.aggregation_method == "AVG"):
                np.save(VARIABLES_FOLDER + '/Q_TABLE_AVG.npy', np.mean(self.end_q_tables_list, axis=0))
                print(f"Saved Q-Table in: {VARIABLES_FOLDER + '/Q_TABLE_AVG.npy'}. ")
            else:  # MAX
                np.save(VARIABLES_FOLDER + '/Q_TABLE_MAX.npy', np.max(self.end_q_tables_list, axis=0))
                print(f"Saved Q-Table in: {VARIABLES_FOLDER + '/Q_TABLE_MAX.npy'}. ")
        ...
        if len(elements_to_remove) > 0:
            for agent in elements_to_remove:
                self.end_q_tables_list.append(agent.control_algorithm.q_table)
    ...
    def get_vehicle_object_by_id(self, vehID):
        ...
        for vhc in self.vehicles_list:
            ...
            self.list_of_vehicles: at the begining of each episode, it's a copy of vehicles_list. However, after each step, it gets updated by removing the 
    ...
    def get_follow_speed_by_id(self, vehID):
        ...
        leader_data = traci.vehicle.getLeader(agent.ID)
    ...
    def calc_blocking(self, communication_range=30, max_blocking_reward=0, min_blocking_reward=-3,
                      max_number_feasible_actions=5, first_veh_perc=0.5, second_veh_perc=1 / 3, third_veh_perc=1 / 6):
        '''

        :param communication_range:  which range ambulance can perceive in front of it based on its sensors , we assume it to be 30  meter or  30 length unit
        :param max_blocking_reward: what is the best reward you can get from not blocking , default is zero it means you are not punished at all
        :param min_blocking_reward: what is the best reward you get from blocking , default is -3 it means you are punished by -3 reward units
        :param max_number_feasible_actions: according to our RL problem , what is max nuumber of actions the agent can perform
        :param first_veh_perc: weight that vehicle contribute to overall blocking , the nearer the vehicle to amb , the more important it become
        that is why first vehicle here 50 % of overall weight
        :param second_veh_perc: second vehicle here 33.3 % of overall weight
        :param third_veh_perc: third vehicle here is 16.7 , the sum of weights of vehicles contribute to blocking is 100 %
        :return:
        '''


        '''we need to check if these vehicles lanes are same as ambulance and their position is
         front of her and no further than its communication range  , then order them from nearest and furthest ,
        '''

        amb_lane = self.list_of_vehicles[0].lane
        amb_abs_pose = self.list_of_vehicles[0].getPose()
        blocking_vehicles = []

        for i in range(len(self.list_of_vehicles[1:])):
            # check if this vehicle in the same lane as ambulance
            lane_check = (self.list_of_vehicles[i + 1].lane == amb_lane)
            # check if this vehicle in front of amb with no more than communication  range
            pose_check = ((self.list_of_vehicles[i + 1].getPose() > amb_abs_pose) & (
                        self.list_of_vehicles[i + 1].getPose() < (amb_abs_pose + communication_range)))
            if (lane_check & pose_check):
                blocking_vehicles.append(self.list_of_vehicles[i + 1])

        blocking_vehicles_sorted = sorted(blocking_vehicles, key=lambda x: x.getPose(), reverse=False)
        # ================================The following printing is for debugging when needed =======================#
        '''
        for i in range(len(blocking_vehicles_sorted)):
             print("Blocking vehicle no : ", i, " | its pose is :", ('%.2f'% blocking_vehicles_sorted[i].getPose()),
                   " | its lane ",  blocking_vehicles_sorted[i].lane,
                   " | num of feasible actions", self.get_feasible_actions(blocking_vehicles_sorted[i]),
                   " | While amb pose is:",  ('%.2f'% amb_abs_pose), " | amb lane", amb_lane)
        if len(blocking_vehicles_sorted) > 0:
            x = len(self.get_feasible_actions(blocking_vehicles_sorted[0]))
            print("feasible_actions", self.get_feasible_actions(blocking_vehicles_sorted[0]))
        '''
        # =============================why x variable has different values based on amb_lane=======================#

        ''' This is to account for that if vehicles in the middle lane , i.e. lane 1 then they could have 
        # CHL , CHR both feasible but if they are in lane 0 then they cannot CHR and similarly if they are in lane 
        2 then they cannot CHL
        '''
        if amb_lane == 1:
            x = 0
        else:
            x = 1
        num_blocking_vehicles = len(blocking_vehicles_sorted)
        # initialize some variables
        first_veh_reward = 0
        second_veh_reward = 0
        third_veh_reward = 0

        ''' the following if statements to handle the fact that amb can 
        have only one car in front of it or two or three and even more '''

        if num_blocking_vehicles > 0:
            num_feasible_actions_first_veh = len(self.get_feasible_actions(blocking_vehicles_sorted[0]))
            m_1 = ((max_blocking_reward - min_blocking_reward) * first_veh_perc) / max_number_feasible_actions
            c_1 = (min_blocking_reward - max_blocking_reward) * first_veh_perc
            first_veh_reward = m_1 * (num_feasible_actions_first_veh + x) + c_1

        if num_blocking_vehicles >= 2:
            num_feasible_actions_second_veh = len(self.get_feasible_actions(blocking_vehicles_sorted[1]))
            m_2 = ((max_blocking_reward - min_blocking_reward) * second_veh_perc) / max_number_feasible_actions
            c_2 = (min_blocking_reward - max_blocking_reward) * second_veh_perc
            second_veh_reward = m_2 * (num_feasible_actions_second_veh + x) + c_2

        if num_blocking_vehicles >= 3:
            num_feasible_actions_third_veh = len(self.get_feasible_actions(blocking_vehicles_sorted[2]))
            m_3 = ((max_blocking_reward - min_blocking_reward) * third_veh_perc) / max_number_feasible_actions
            c_3 = (min_blocking_reward - max_blocking_reward) * third_veh_reward
            third_veh_reward = m_3 * (num_feasible_actions_third_veh + x) + c_3

        blocking_reward = first_veh_reward + second_veh_reward + third_veh_reward
        # for debugging
        #print(" Blocking reward ", ('%.2f' % blocking_reward))

        return blocking_reward

    def calc_reward(self, amb_last_velocity, done, number_of_steps, max_final_reward=20, min_final_reward=-20, max_amb_step_reward=0, min_amb_step_reward=-5,
                    timestep_reward = -1 ):
        ...
        :param max_amb_step_reward: reward for having an acceleration of value = self.emer.max_accel (=2) over last step
        :param min_amb_step_reward: reward for having an acceleration of value = - self.emer.max_accel (=- 2) over last step
        ...
        if(done and give_final_reward): #Calculate a final reward
            ...
            m = (max_amb_step_reward - min_amb_step_reward) / (2 * self.emer.max_accel)  # Slope for straight line equation to calculate step reward


            #2 * self.emer.max_accel since: = self.emer.max_accel - * self.emer.max_decel
            c = max_amb_step_reward - self.emer.max_accel * m  # c is y-intercept for the reward function equation #max_step_reward is the y for x = 2 (max acceleration)

            amb_acc_reward = m * (self.emer.spd - amb_last_velocity) + c
            ...
            #   will be wrongly assumed to be zero. Although the ambulance probably could have accelerated more, but this is its maximum velocity.
                amb_acc_reward = max_amb_step_reward #same reward as maximum acceleration (+2),

            blocking_reward = self.calc_blocking()   # to take into account when other vehicles block amb way

            reward = amb_acc_reward + timestep_reward + blocking_reward  # global reward
            #for debugging
            '''
            print("amb_last_velocity : ", amb_last_velocity, " | current_velocity:", (self.emer.spd),
                  " | blocking reward  ", blocking_reward,
                  " | global reward", reward, " amb_acc_reward", ('%.2f' % amb_acc_reward))
            '''
            self.reward = reward
            return reward
