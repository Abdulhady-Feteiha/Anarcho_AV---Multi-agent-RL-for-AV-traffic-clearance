...
class SimTools():
    ...
    def episode(sumoBinary, Proudhon=None, episode_num=0):
        ...
        # ambulance does not have observed state
        ...
        while traci.simulation.getMinExpectedNumber() > 0:
            ...
            num_exPLOIT = sum([1 for vhcl in Proudhon.list_of_vehicles if vhcl.control_algorithm.action_chosing_method == "expLOIT" and vhcl.type != "Emergency"])
            num_exPLORE = len(Proudhon.list_of_vehicles) - 1 - num_exPLOIT
            nums_str = f"exPLOIT/exPlore " + fill_str(f"{num_exPLOIT}:{num_exPLORE}", 10) +' |',
            ...
            if (step % vis_update_params['every_n_iters'] == 0 and episode_num % vis_update_params['every_n_episodes'] == 0): # print step info   #TempComment
                print(f'E:{episode_num: <{6}}|S:{step: <{4}} |' #TempComment
                      ...
                      f'NumVehicles: {fill_str(str(len(Proudhon.list_of_vehicles)), 5)} |',
                      nums_str,
                      f'CumReward: ' + str(episode_reward)[:6] + ' ' * max(0,
                                                                                6 - len(str(episode_reward))) + " | ")

                # ----------------------------------------------------------------- #

            #Proudhon.calc_blocking()  #for debugging only calc_blocking
            ...
            done = Proudhon.are_we_done(step_number=step)  # !! NOTE: INISIDE THIS FUNCTION WE REMOVE VEHICLES, AND SAVE THEIR Q_TABLES BEFORE THEY LEAVE !!
            ...
            for i, vhcl in enumerate(Proudhon.list_of_vehicles):
                if (vhcl.type != "Emergency"):  # i.e. skip i =0, start at i =1
                    vhcl.control_algorithm.update_q_table(chosen_action[i-1], reward, new_observed_state[i-1],
                                           last_observed_state[i-1], feasible_actions_for_current_state[i])
        ...
        q_learning_params['epsilon'] = q_learning_params['min_epsilon'] + (q_learning_params['max_epsilon'] - q_learning_params['min_epsilon']) * \
                                       np.exp(-q_learning_params['decay_rate'] * episode_num)
