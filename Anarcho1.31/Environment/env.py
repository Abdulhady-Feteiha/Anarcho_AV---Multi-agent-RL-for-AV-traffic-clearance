...
class env():
    ...
    def __init__(self, list_of_vehicles, name="SingleAgentEvn1.0",  ambulance_goal_distance=500, rel_amb_y_min = -41, rel_amb_y_max = 16 ):
        ...
        self.reward = None # rl_disengagement should be None  # calc_reward handle rl initialization
    ...
    def reset(self):
        ...
        * Commented lines are tautologies (do not add new info), kept only for reference to what is inherited from last
        ...
        #new_stance['agent_start_lane'] = 0
        #new_stance['agent_start_lane'] = 0
        new_stance['agent_start_lane'] = 1
        ...
        #new_stance['ambulance_start_lane'] = 0
        #new_stance['ambulance_start_lane'] = 0
        new_stance['ambulance_start_lane'] = 2
    ...
    def get_feasible_actions(self, agent ,observed_state_for_agent):
        ...
        rel_distance = observed_state_for_agent[4]           # to determine when rl disengagement is applied
        if(enable_rl_dis_engagement & (~ ( (rel_distance < self.rel_amb_y_max ) & (rel_distance > self.rel_amb_y_min )))):

            # if  the agent is  outside ambulance window and rl _disenagement is enabled  , disable RL and do not investigate which actions are feasible
    ...
    def calc_reward(self, amb_last_velocity, done, number_of_steps,previous_state , current_state,
        ...
        else:     #  it means if it  is a step reward

            rel_distance = current_state[4]
            if (enable_rl_dis_engagement & (~ ((rel_distance < self.rel_amb_y_max) & (rel_distance > self.rel_amb_y_min)))):
                # if  the agent is  outside ambulance window and rl _disenagement is enabled  , disable RL and make reward equal None
